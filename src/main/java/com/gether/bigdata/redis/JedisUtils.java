...
public class JedisUtils {
		...
		try {
			...
			if (jedis.exists(getBytesKey(key))) {
				...
				for (Map.Entry<byte[], byte[]> e : map.entrySet()) {
					value.put(byte2String(e.getKey()), toObject(e.getValue()));
				}
	...
	public static Long hincrByVal(String key, String fielId, long value) {
		...
		try {
			jedis = getResource();
			result = jedis.hincrBy(key, fielId, value);
	 ...
	 * @return
	 */
	public static void hdelete(String key, String... fielId) {
		Jedis jedis = null;
		try {
			jedis = getResource();
			jedis.hdel(key, fielId);
			logger.debug("hdelete  key:{}, fielId:{}", key, fielId);
		} catch (Exception e) {
			logger.warn("hincrBy  key:{}, fielId:{}", key, fielId, e);
		} finally {
			returnResource(jedis);
