...
public class NetflixArchiusTest {
  ...
  public void testFileConfig() throws IOException, InterruptedException {
    ...
    configuration.addConfigurationListener(new ConfigurationListener() {
      ...
      public void configurationChanged(ConfigurationEvent event) {
        if (!event.isBeforeUpdate()) {
          System.err.println(JSON.toJSONString(event));
        }
      }
    });
    ...
    while (true) {
      Thread.sleep(1000);
      DynamicStringProperty message = DynamicPropertyFactory.getInstance()
          .getStringProperty("spring.http.encoding.force", null);
  ...
  public void testMultiConfig() throws IOException, InterruptedException {
    ...
    FixedDelayPollingScheduler scheduler2 = new FixedDelayPollingScheduler(0, 60 * 1000,
        false); //60s 60*1000
  ...
  public void testSelfDefineConfig() throws InterruptedException {
    ...
    FixedDelayPollingScheduler scheduler = new FixedDelayPollingScheduler(0, 60 * 1000,
        false); //60s 60*1000
    DynamicConfiguration configuration = new DynamicConfiguration(source, scheduler);
    ...
    while (true) {
      Thread.sleep(1000);
      DynamicStringProperty message = DynamicPropertyFactory.getInstance()
          .getStringProperty("syspara", null);
      System.out.println("message:" + message);
    }
  }
